 ...
 * Files relative path defined at
 * {@link cn.gy.test.configs.constants.ConstantBasic.CONFIG_FILE} <br>
 * Read files by prefix path
 * {@link cn.gy.test.configs.constants.ConstantBasic.userDir} + relative path
 * 
 * @author guyan 2016年3月20日
 */
    ...
    synchronized public static void init(String... files) {
        for (int i = 0; i < files.length; i++) {
            ...
            if (filePath.length() > 0) {
                Properties properties = IOUtils.getPropertiesFromFile(ConstantBasic.userDir + File.separator + filePath,
                        DEFAULT_ENCODING);
                if (null != properties) {
